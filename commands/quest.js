const { SlashCommandBuilder, EmbedBuilder } = require('discord.js'); //Client, GatewayIntentBits, Partials, ButtonBuilder, ActionRowBuilder
...
module.exports = {
	...
	async execute(interaction) {
		...
		try {
			...
			if (!choice) {
				await displayQuestBoard(interaction, dailyQuests, query);
			} else {
...
async function handle20PercentQuest(interaction, quest, query) {
	const userId = interaction.user.id;
...
async function handleFidelityQuest(interaction, quest, query, choice) {
	const userId = interaction.user.id;
	if (quest.id === 'x') {
...
async function completeFidelityQuest(interaction, quest, query, userId, choice) {
	const playerQuestsResults = await query(`SELECT * FROM player_quests WHERE discord_id = '${userId}'`);
	const nowEpochInSeconds = ~~(new Date().getTime() / 1000);
	const midnight = new Date();
	midnight.setHours(24, 0, 0, 0);
